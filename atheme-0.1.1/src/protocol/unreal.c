 ...
 *
 * $Id: unreal.c 329 2005-06-04 21:20:38Z nenolod $
 */
...
uint8_t server_login(void)
{
        ...
        ret = sts("PASS %s", me.pass);
        if (ret == 1)
                return 1;

        me.bursting = TRUE;

        sts("PROTOCTL NICKv2 VHP UMODE2 SJOIN SJOIN2 SJ3 NOQUIT TKLEXT");
...
void kline_sts(char *server, char *user, char *host, long duration, char *reason)
{
	...
	sts(":%s TKL + G %s %s %s 0 %ld :%s", me.name, user, host, chansvs.nick, time(NULL), 
		reason);
}
...
void unkline_sts(char *server, char *user, char *host)
{
	...
	sts(":%s TKL - G %s %s %s", me.name, user, host, chansvs.nick);
}
...
static void m_nick(char *origin, uint8_t parc, char *parv[])
{
	...
	if (parc == 10)
	{
		...
		user_add(parv[0], parv[3], parv[4], s);

		user_mode(user_find(parv[0]), parv[7]);
