...
command_t cs_register = { "REGISTER", "Registers a username or channel.",
...
static void cs_cmd_register(char *origin)
{
	...
	else if (nicksvs.enable != TRUE)
	{
		...
		if (mu != NULL)
		{
			...
			if ((!(mu->flags & MU_HIDEMAIL))
				...
				notice(chansvs.nick, origin, "\2%s\2 is already registered.", mu->name);
	...
	else
	{
		...
		notice(chansvs.nick, origin, "To do so, use: /%s%s REGISTER <password> <email>.", 
			(ircd->uses_rcommand == TRUE) ? "msg " : "", nicksvs.disp);
	}
}
