...
static void cs_set_founder(char *origin, char *name, char *params)
{
	...
	/* we need a way to kill all access for here and FTRANSFER */
	chanacs_delete(mc, mc->founder, CA_FOUNDER);
	chanacs_delete(mc, u->myuser, CA_VOP);
	chanacs_delete(mc, u->myuser, CA_HOP);
...
static void cs_set_property(char *origin, char *name, char *params)
{
	...
	mychan_t *mc;
        ...
        if (!property)
        {
                notice(chansvs.nick, origin, "Syntax: SET <username|#channel> PROPERTY <property> [value]");
                return;
        }
