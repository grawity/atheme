...
static void postgresql_db_save(void *arg)
{
	...
	for (i = 0; i < HASHSIZE; i++)
	{
		...
		{
			char cname[BUFSIZE], cpass[BUFSIZE], cfounder[BUFSIZE];
			char ckey[BUFSIZE], curl[BUFSIZE], centrymsg[BUFSIZE];

			mc = (mychan_t *)n->data;

			PQescapeString(cname, mc->name, BUFSIZE);
			PQescapeString(cpass, mc->pass, BUFSIZE);
			...
			if (mc->url)
				PQescapeString(curl, mc->url, BUFSIZE);
			if (mc->entrymsg)
				PQescapeString(centrymsg, mc->entrymsg, BUFSIZE);
                        if (mc->url)
                                PQescapeString(curl, mc->url, BUFSIZE);
			...
			res = safe_query("INSERT INTO CHANNELS(ID, NAME, PASSWORD, FOUNDER, REGISTERED, LASTUSED, "
					...
					cname, cpass, cfounder, (long)mc->registered, (long)mc->used, (long)mc->flags,
					(long)mc->mlock_on, (long)mc->mlock_off, (long)mc->mlock_limit,
					mc->mlock_key ? ckey : "", mc->url ? curl : "", mc->entrymsg ? centrymsg : "");
...
static void postgresql_metadata_delete(void *target, int32_t type, char *name)
{
        node_t *n;
        myuser_t *mu;   
        mychan_t *mc;
