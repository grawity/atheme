 ...
 *
 * $Id: undernet.c 902 2005-07-17 02:26:34Z alambert $
 */
...
/* PRIVMSG wrapper */
static void undernet_msg(char *target, char *fmt, ...)
{
        va_list ap;
        char buf[BUFSIZE];
...
static void m_stats(char *origin, uint8_t parc, char *parv[])
{
	...
	switch (parv[0][0])
	{
	  case 'C':
	  case 'c':
		  sts("%s 213 %s C *@127.0.0.1 A %s %d uplink", curr_uplink->numeric, u->nick, (is_ircop(u)) ? curr_uplink->name : "127.0.0.1", me.port);
		  break;
	  ...
	  case 'e':
		  ...
		  sts("%s 249 %s E :Last event to run: %s", curr_uplink->numeric, u->nick, last_event_ran);

		  for (i = 0; i < MAX_EVENTS; i++)
		  {
			  if (event_table[i].active)
				  sts("%s 249 %s E :%s (%d)", curr_uplink->numeric, u->nick, event_table[i].name, event_table[i].frequency);
		  }
	  ...
	  case 'H':
	  case 'h':
		  sts("%s 244 %s H * * %s", curr_uplink->numeric, u->nick, (is_ircop(u)) ? curr_uplink->name : "127.0.0.1");
		  break;

	  case 'I':
	  case 'i':
		  sts("%s 215 %s I * * *@%s 0 nonopered", curr_uplink->numeric, u->nick, me.name);
		  break;
	  ...
	  case 'K':
		  ...
		  LIST_FOREACH(n, klnlist.head)
		  {
			  ...
			  if (!k->duration)
				  sts("%s 216 %s K %s * %s :%s", curr_uplink->numeric, u->nick, k->host, k->user, k->reason);
		  }
	  ...
	  case 't':
		  ...
		  sts("%s 249 %s :event      %7d", curr_uplink->numeric, u->nick, cnt.event);
		  sts("%s 249 %s :sra        %7d", curr_uplink->numeric, u->nick, cnt.sra);
		  sts("%s 249 %s :tld        %7d", curr_uplink->numeric, u->nick, cnt.tld);
		  sts("%s 249 %s :kline      %7d", curr_uplink->numeric, u->nick, cnt.kline);
		  sts("%s 249 %s :server     %7d", curr_uplink->numeric, u->nick, cnt.server);
		  sts("%s 249 %s :user       %7d", curr_uplink->numeric, u->nick, cnt.user);
		  sts("%s 249 %s :chan       %7d", curr_uplink->numeric, u->nick, cnt.chan);
		  sts("%s 249 %s :chanuser   %7d", curr_uplink->numeric, u->nick, cnt.myuser);
		  sts("%s 249 %s :mychan     %7d", curr_uplink->numeric, u->nick, cnt.mychan);
		  sts("%s 249 %s :chanacs    %7d", curr_uplink->numeric, u->nick, cnt.chanacs);
		  sts("%s 249 %s :node       %7d", curr_uplink->numeric, u->nick, cnt.node);

		  sts("%s 249 %s :bytes sent %7.2f%s", curr_uplink->numeric, u->nick, bytes(cnt.bout), sbytes(cnt.bout));
		  sts("%s 249 %s :bytes recv %7.2f%s", curr_uplink->numeric, u->nick, bytes(cnt.bin), sbytes(cnt.bin));
		  break;

	  case 'u':
		  sts("%s 242 %s :Services Up %s", curr_uplink->numeric, u->nick, timediff(CURRTIME - me.start));
		  break;
	...
	sts("%s 219 %s %c :End of STATS report", curr_uplink->numeric, u->nick, parv[0][0]);
}
